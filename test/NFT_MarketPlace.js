...
describe('NFT Marketplace', () => {
    let add1, add2,add3, addrs, deployer, NFT, nft,Marketplace,marketPlace
    let URI = 'http;//smaple-URI'
    let feePercent = 10
    beforeEach(async () => {
        [deployer,buyer, seller, add1, add2, add3, ...addrs] = await ethers.getSigners()
    ...
    describe('Purchase Item from Market Place',()=>{
        ...
        it('Should update item as sold, pay seller, transfer NFT to buyer, charge fees and emit a Bought event',async()=>{
            ...
            console.log('feeAccountFinalBalance :'+fromWei(feeAccountFinalBalance))
                console.log(await marketPlace.items(1))
            await nft.connect(add2).setApprovalForAll(marketPlace.address,true)
            const seller = await marketPlace.connect(add2).sellItem(1,toWei(30))
            console.log(await marketPlace.items(1))  
